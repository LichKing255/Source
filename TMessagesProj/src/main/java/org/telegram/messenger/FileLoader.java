...
public class FileLoader
{
    ...
    this.fileLoaderQueue.postRunnable(new Runnable()
    {
      public void run()
      {
        ...
        if (MessageObject.isVoiceDocument(paramDocument)) {
          FileLoader.this.audioLoadOperationQueue.remove(localObject);
        }
        for (;;)
        {
          ...
          return;
          if (paramFileLocation != null) {
            FileLoader.this.photoLoadOperationQueue.remove(localObject);
          } else {
            FileLoader.this.loadOperationQueue.remove(localObject);
          }
        }
      }
    });
  ...
  private void checkDownloadQueue(final TLRPC.Document paramDocument, final TLRPC.FileLocation paramFileLocation, final String paramString)
  {
    this.fileLoaderQueue.postRunnable(new Runnable()
    {
      ...
      {
        int i = 3;
        FileLoader.this.loadOperationPaths.remove(paramString);
        FileLoadOperation localFileLoadOperation;
        if (MessageObject.isVoiceDocument(paramDocument))
        {
          FileLoader.access$1410(FileLoader.this);
          if (!FileLoader.this.audioLoadOperationQueue.isEmpty())
          {
            ...
            if (FileLoader.this.currentAudioLoadOperationsCount < i)
            {
              ...
              {
                FileLoader.access$1408(FileLoader.this);
                localFileLoadOperation.start();
              }
            }
          }
        }
        ...
        for (;;)
        {
          ...
          {
            FileLoader.access$1610(FileLoader.this);
            if (!FileLoader.this.loadOperationQueue.isEmpty())
            {
              ...
              for (;;)
              {
                ...
                FileLoader.access$1608(FileLoader.this);
                localFileLoadOperation.start();
                return;
                i = 1;
              }
...
/* Location:              C:\Users\Armandl\Downloads\Compressed\dex2jar-2.0\classes-dex2jar.jar!\org\telegram\messenger\FileLoader.class
