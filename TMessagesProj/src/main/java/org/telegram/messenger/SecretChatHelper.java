...
public class SecretChatHelper
{
  ...
  protected ArrayList<TLRPC.Message> decryptMessage(final TLRPC.EncryptedMessage paramEncryptedMessage)
  {
    ...
    for (;;)
    {
      ...
      label883:
      ((NativeByteBuffer)localObject3).reuse();
      FileLog.e("tmessages", "fingerprint mismatch " + l);
    }
    return null;
  }
                          ...
                          public void run()
                          {
                            ...
                            AndroidUtilities.runOnUIThread(new Runnable()
                            {
                              public void run()
                              {
                                ...
                                if (MessageObject.isVideoMessage(SecretChatHelper.4.this.val$newMsgObj)) {
  ...
  public TLRPC.Message processDecryptedObject(TLRPC.EncryptedChat paramEncryptedChat, final TLRPC.EncryptedFile paramEncryptedFile, int paramInt, long paramLong, TLObject paramTLObject, boolean paramBoolean)
  {
    Object localObject1;
    label173:
    label465:
    ...
    for (;;)
    {
      ...
      else
      {
        ...
        else if ((((TLRPC.TL_decryptedMessageService)localObject2).action instanceof TLRPC.TL_decryptedMessageActionAbortKey))
        {
          ...
          {
            paramEncryptedFile = new TLRPC.TL_encryptedChatDiscarded();
            paramEncryptedFile.id = paramEncryptedChat.id;
            paramEncryptedFile.user_id = paramEncryptedChat.user_id;
            paramEncryptedFile.auth_key = paramEncryptedChat.auth_key;
            paramEncryptedFile.key_create_date = paramEncryptedChat.key_create_date;
            paramEncryptedFile.key_use_count_in = paramEncryptedChat.key_use_count_in;
            paramEncryptedFile.key_use_count_out = paramEncryptedChat.key_use_count_out;
            paramEncryptedFile.seq_in = paramEncryptedChat.seq_in;
            paramEncryptedFile.seq_out = paramEncryptedChat.seq_out;
            MessagesStorage.getInstance().updateEncryptedChat(paramEncryptedFile);
            AndroidUtilities.runOnUIThread(new Runnable()
            {
              public void run()
              {
                MessagesController.getInstance().putEncryptedChat(paramEncryptedFile, false);
                NotificationCenter.getInstance().postNotificationName(NotificationCenter.encryptedChatUpdated, new Object[] { paramEncryptedFile });
              }
            });
            declineSecretChat(paramEncryptedChat.id);
          }
  ...
  protected void processUpdateEncryption(TLRPC.TL_updateEncryption paramTL_updateEncryption, final ConcurrentHashMap<Integer, TLRPC.User> paramConcurrentHashMap)
  {
    ...
    do
    {
      ...
      if ((localObject != null) && ((localObject instanceof TLRPC.TL_encryptedChatWaiting)) && ((((TLRPC.EncryptedChat)localObject).auth_key == null) || (((TLRPC.EncryptedChat)localObject).auth_key.length == 1)))
      {
        ...
        getInstance().processAcceptedSecretChat(localEncryptedChat);
        return;
      }
            ...
            public void run(final TLObject paramAnonymous2TLObject, TLRPC.TL_error paramAnonymous2TL_error)
            {
              if (paramAnonymous2TL_error == null)
              {
                AndroidUtilities.runOnUIThread(new Runnable()
                {
                  public void run()
                  {
                    SecretChatHelper.access$302(SecretChatHelper.this, false);
                    if (!((Activity)SecretChatHelper.14.this.val$context).isFinishing()) {}
                    try
                    {
                      SecretChatHelper.14.this.val$progressDialog.dismiss();
              ...
              AndroidUtilities.runOnUIThread(new Runnable()
              {
                public void run()
                {
                  ...
                  try
                  {
                    ...
                    localBuilder.setMessage(LocaleController.getString("CreateEncryptedChatError", 2131165576));
...
/* Location:              C:\Users\Armandl\Downloads\Compressed\dex2jar-2.0\classes-dex2jar.jar!\org\telegram\messenger\SecretChatHelper.class
